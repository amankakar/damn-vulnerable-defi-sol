...
describe('[Challenge] Puppet', function () {
    ...
    it('Execution', async function () {
        ...
        const attacker = await (await ethers.getContractFactory('PuppetAttack', deployer)).deploy(
            token.address, player.address, uniswapExchange.address, lendingPool.address, { value: ethers.utils.parseEther("15") }
        )
        await token
            .connect(player)
            .transfer(attacker.address, PLAYER_INITIAL_TOKEN_BALANCE);
        await attacker.attack();
        console.log("token in pool", ethers.utils.formatEther((await token.balanceOf(lendingPool.address)).toString()));
